...
class DecoderPitch(nn.Module):
    def __init__(self, n_z=16,n_hidden=512):
        super().__init__()

        self.mlp_pitch = self.make_mlp(1,n_hidden)
        self.mlp_res = self.make_mlp(n_z,n_hidden)
        self.gru = nn.GRU(n_hidden*2,n_hidden)
        self.mlp_out = self.make_mlp(n_hidden,n_hidden)
        self.dense = nn.Linear(n_hidden,1)
    ...
    def forward(self, pitch,res):
        pitch_out = self.mlp_pitch(pitch)
        res  = self.mlp_res(res)
        out,_  = self.gru(torch.cat([pitch_out,res],-1))
        
        out =self.mlp_out(out)
        out = self.dense(out)
        return (pitch*(1+torch.tanh(out)))
