...
class Decoder(nn.Module):
    def __init__(self, sr,n_amps,n_bands,residual=False,n_z=16,n_hidden=512):
        ...
        self.register_buffer("cache_gru", torch.zeros(1, 1, n_hidden))
    ...
    def real_time_forward(self, pitch,loud,res=None):
        pitch_out = self.mlp_pitch(pitch)
        loud_out  = self.mlp_loud(loud)
        if self.residual == True:
            res_out = self.mlp_res(res)
            gru_out,cache_gru  = self.gru(torch.cat([pitch_out,loud_out,res_out],-1),self.cache_gru)
        else:
            gru_out,cache_gru  = self.gru(torch.cat([pitch_out,loud_out],-1),self.cache_gru)
